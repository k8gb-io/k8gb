...
|------------|------|---------|
| https://coredns.github.io/helm | coredns | 1.46.0 |
...
| k8gb.edgeDNSExposedIPs |  array  | `[]` | cluster-level override of the publicly routable IPv4 addresses this cluster is exposed on. When set, these IPs are published both as the zone-delegation NS glue records and as the application localtargets-*/final A records, in place of the IPs discovered from the CoreDNS service/ingress. Per-Gslb annotations (k8gb.io/exposed-ip-addresses, k8gb.io/exposed-hostnames) take precedence over this override. Use on bare-metal/colo clusters behind 1:1 static NAT that only see private node IPs. Leave empty to keep the default auto-discovery behavior. |
...
| tracing.sidecarImage.tag | string | `"0.155.0"` |  |
