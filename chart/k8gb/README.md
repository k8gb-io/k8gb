...
| k8gb.ingressController | object | `{"healthCheck":{"enabled":false}}` | Ingress controller health check configuration |
| k8gb.ingressController.healthCheck.enabled | bool | `false` | Enable health checking for the ingress controller/load balancer. When enabled, k8gb considers both application health and ingress controller health in failover decisions. The ingress controller service is automatically discovered based on the ingress type. |
