...
// Random picks random DNSServer (roundrobin over dnsservers) and returns !found if DNSList is empty
func (l DNSList) Random() (*DNSServer, bool) {
	if len(l) == 0 {
		return nil, false
	}
	randomIndex := rnd.Intn(len(l))
	return &l[randomIndex], true
}
