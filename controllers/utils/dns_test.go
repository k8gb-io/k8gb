...
func TestValidDigFQDNWithDot(t *testing.T) {
	...
	fqdn := defaultFqdn
	// act
	result, err := Dig(fqdn+".", 8, defaultEdgeDNSServer)
	// assert
...
func TestEmptyEdgeDNS(t *testing.T) {
	...
	fqdn := "whatever"
	// act
	result, err := Dig(fqdn, 8, DNSServer{Host: "", Port: 53})
	// assert
	assert.Error(t, err)
	assert.Nil(t, result)
}
...
func TestEmptyDNSList(t *testing.T) {
	...
	fqdn := "whatever"
	// act
	result, err := Dig(fqdn, 8, []DNSServer{}...)
	// assert
	assert.Error(t, err)
	assert.Nil(t, result)
}

func TestValidDigButMaxRecursion(t *testing.T) {
...
func TestValidEdgeDNSButNonExistingFQDN(t *testing.T) {
	...
	// act
	result, err := Dig(fqdn, 8, DNSServer{Host: edgeDNSServer, Port: 53})
	// assert
	assert.Error(t, err)
	assert.Nil(t, result)
}

func TestDigCNAME(t *testing.T) {
	testServer := DNSServer{
		Host: server,
		Port: port,
	}
