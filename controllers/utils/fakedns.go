...
type DNSMock struct {
	...
	extraRecords   map[string][]dns.RR
	server         *dns.Server
	err            error
}
...
func NewFakeDNS(settings FakeDNSSettings) *DNSMock {
	return &DNSMock{
		...
		extraRecords:   make(map[string][]dns.RR),
...
func (m *DNSMock) AddExtraARecord(queryFQDN string, recordFQDN string, ip net.IP) *DNSMock {
	rr := &dns.A{
		Hdr: dns.RR_Header{
			Name:   recordFQDN,
			Rrtype: dns.TypeA,
	...
	m.extraRecords[queryFQDN] = append(
		m.extraRecords[queryFQDN],
		rr,
	)
...
func (m *DNSMock) handleReflect(w dns.ResponseWriter, r *dns.Msg) {
	...
	if extra := m.extraRecords[question.Name]; extra != nil {
