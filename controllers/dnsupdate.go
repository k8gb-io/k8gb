...
func (r *GslbReconciler) gslbDNSEndpoint(gslb *k8gbv1beta1.Gslb) (*externaldns.DNSEndpoint, error) {
	...
	for host, health := range gslb.Status.ServiceHealth {
		...
		if !r.Config.DelegationZones.ContainsZone(host) {
			return nil, fmt.Errorf("ingress host %s does not match delegated zone %v", host, r.Config.DelegationZones.ListZones())
		}
