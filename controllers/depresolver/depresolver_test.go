...
func TestNsServerNamesLargeDNSZone(t *testing.T) {
	...
	customConfig := predefinedConfig
	customConfig.dnsZones = "cloud.example.com:k8gb-test-preprod-lorem-ipsum-donor-blah-blah-blah.gslb.cloud.example.com"
	...
	// assert
	assert.Error(t, err)
	assert.Equal(t, "gslb-ns-us-k8gb-test-preprod-lorem-ipsum-donor-blah-blah-blah-gslb.cloud.example.com", config.DelegationZones[0].ClusterNSName)
	extNsNames := config.DelegationZones[0].ExtClusterNSNames
