	...
	coreDNSServiceHandler := handler.EnqueueRequestsFromMapFunc(
		func(ctx context.Context, object client.Object) []reconcile.Request {
			...
			if svc.Spec.Type == corev1.ServiceTypeLoadBalancer {
				ips, err = assistant.NewGslbAssistant(r.Client, r.Config.K8gbNamespace, *r.Config).CoreDNSExposedIPs()
				if err != nil {
					log.Fatal().Err(err).Msg("Can't fetch IP addresses from CoreDNS service type LoadBalancer")
				}
				if len(ips) == 0 {
					log.Fatal().Msg("No IP's bound in CoreDNS service type LoadBalancer")
				}
			} else {
				ingressPath, found := svc.Annotations["k8gb.io/coredns-ingressStr-ref"]
				if !found {
					log.Fatal().Msg("Missing \"k8gb.io/coredns-ingressStr-ref\" annotation in CoreDNS service")
				}
				ips, err = assistant.NewIngressFiner(ctx, r.Client).Find(ingressPath)
				if err != nil {
					log.Fatal().Err(err).Msg("Can't fetch IP addresses from CoreDNS service type Ingress")
				}
			}

			for _, dz := range r.Config.DelegationZones {
				err = r.DNSProvider.CreateZoneDelegation(&dz, ips)
				if err != nil {
					log.Fatal().Err(err).Msg("Can't create zone delegation")
				}
			}
			return nil
		})
