...
func TestConfigurations(t *testing.T) {
	var tests = []struct {
		...
		},
		{
			...
			assert: func(t *testing.T, cfg *Config) {
				m := ClusterNSNames{
					"gslb-ns-eu-cloud.example.com": NewGeoTag(false, "eu"),
					"gslb-ns-za-cloud.example.com": NewGeoTag(false, "za"),
				}
		...
		{
			name: "valid configuration with cluster exposed IPs override",
			envvars: `CLUSTER_GEO_TAG=us;
				 DNS_ZONES=example.com:cloud.example.com:300;
				 EDGE_DNS_SERVERS=local.test;
				 CLUSTER_EXPOSED_IPS=203.0.113.10,203.0.113.11`,
			assert: func(t *testing.T, cfg *Config) {
				assert.Equal(t, []string{"203.0.113.10", "203.0.113.11"}, cfg.ClusterExposedIPs)
			},
		},
		{
			name: "empty cluster exposed IPs override is valid",
			envvars: `CLUSTER_GEO_TAG=us;
				 DNS_ZONES=example.com:cloud.example.com:300;
				 EDGE_DNS_SERVERS=local.test`,
			assert: func(t *testing.T, cfg *Config) {
				assert.Empty(t, cfg.ClusterExposedIPs)
			},
		},
		{
			name: "invalid configuration with non-IP cluster exposed IPs override",
			envvars: `CLUSTER_GEO_TAG=us;
				 DNS_ZONES=example.com:cloud.example.com:300;
				 EDGE_DNS_SERVERS=local.test;
				 CLUSTER_EXPOSED_IPS=203.0.113.10,not-an-ip`,
			expectedError: true,
		},
		{
			name: "invalid configuration with non-unique cluster exposed IPs override",
			envvars: `CLUSTER_GEO_TAG=us;
				 DNS_ZONES=example.com:cloud.example.com:300;
				 EDGE_DNS_SERVERS=local.test;
				 CLUSTER_EXPOSED_IPS=203.0.113.10,203.0.113.10`,
			expectedError: true,
		},
		{
			name: "invalid configuration with IPv6 cluster exposed IPs override",
			envvars: `CLUSTER_GEO_TAG=us;
				 DNS_ZONES=example.com:cloud.example.com:300;
				 EDGE_DNS_SERVERS=local.test;
				 CLUSTER_EXPOSED_IPS=2001:db8::1`,
			expectedError: true,
		},
	}
