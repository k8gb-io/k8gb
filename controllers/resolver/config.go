...
type Config struct {
	...
	ClusterExposedIPs []string `env:"CLUSTER_EXPOSED_IPS" optional:"" validate:"omitempty,unique,dive,ipv4" help:"cluster-level override of the publicly routable IP addresses this cluster is exposed on; comma-separated. When set, these IPs are published both as the zone-delegation NS glue records and as the application localtargets-*/final A records, in place of the IPs discovered from the CoreDNS service/ingress. Each IP must route DNS traffic to CoreDNS and application traffic to the cluster ingress. Per-Gslb annotations (k8gb.io/exposed-ip-addresses, k8gb.io/exposed-hostnames) take precedence and must be used when applications have different public IPs. IPv6 is rejected because both record types publish A records only. Use on bare-metal/colo clusters that sit behind 1:1 static NAT and only see private node IPs. e.g: '203.0.113.10,203.0.113.11'"`
