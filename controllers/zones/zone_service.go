...
type ZoneDelegation interface {
	...
	RemoveZoneDelegation(ctx context.Context, zd *v1beta1.ZoneDelegation, provider dns.Provider) error
}
...
func (z *ZoneDelegationImpl) RemoveZoneDelegation(ctx context.Context, zd *v1beta1.ZoneDelegation, provider dns.Provider) error {

	isLastAliveCluster := func(zd *v1beta1.ZoneDelegation) bool {
		current := z.config.GetNsName(zd.Spec.LoadBalancedZone, zd.Spec.ParentZone)
	...
	canDeleteZoneDelegation := isLastAliveCluster(zd)
	err = provider.Finalize(zd, canDeleteZoneDelegation)
