...
func (p *ExternalDNSProvider) Finalize(zoneInfo *v1beta1.ZoneDelegation, finalize bool) error {
	if !finalize {
		log.Info().Msgf("Domain %s will be deleted by removing delegation DNSEndpoint", zoneInfo.Spec.LoadBalancedZone)
	}
	return nil
}
