...
// Custom FakeClient that returns NotFound after the first Get call
type FakeClientWithFailingGet struct {
	client.Client
	mu       sync.Mutex
	getCount int
}
...
func (f *FakeClientWithFailingGet) Get(ctx context.Context, key client.ObjectKey, obj client.Object) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	...
	f.getCount++
	return f.Client.Get(ctx, key, obj) // Call the original fake client Get
}
