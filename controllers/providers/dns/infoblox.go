...
func (p *InfobloxProvider) Finalize(zoneInfo *v1beta1.ZoneDelegation, finalize bool) error {
	if !finalize {
		log.Info().Msgf("Zone %s must deleted by manually in Infoblox", zoneInfo.Spec.LoadBalancedZone)
	}
	objMgr, err := p.client.GetObjectManager()
	if err != nil {
		return err
	}
	findZone, err := p.getZoneDelegated(objMgr, zoneInfo.Spec.LoadBalancedZone)
	if err != nil {
		return err
	}
	_, err = p.deleteZoneDelegated(objMgr, findZone.Fqdn)
	if err != nil {
		return fmt.Errorf("removing zonedelegation %s from infoblox (%s)", zoneInfo.Spec.LoadBalancedZone, err)
	}
	return nil
}
