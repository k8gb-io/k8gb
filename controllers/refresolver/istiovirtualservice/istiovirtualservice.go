...
func getGslbVirtualServiceRef(gslb *k8gbv1beta1.Gslb, k8sClient client.Client) ([]*istio.VirtualService, error) {
	...
	if reflect.DeepEqual(gslb.Spec.ResourceRef.IstioVirtualService, metav1.LabelSelector{}) {
		log.Warn().
			Str("gslb", gslb.Name).
	...
	selector, err := metav1.LabelSelectorAsSelector(&gslb.Spec.ResourceRef.IstioVirtualService)
	if err != nil {
		return nil, err
	}
...
// getLbService retrieves the kubernetes service referenced by the istio gateway
...
// GetServers retrieves the server configuration of the GSLB from the istio virtual service resource
