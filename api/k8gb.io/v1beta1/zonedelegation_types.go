...
func (d *ZoneDelegationList) ListZones() []string {
	var zones []string
	for _, z := range d.Items {
		zones = append(zones, z.Spec.LoadBalancedZone)
	}
	return zones
}

func (d *ZoneDelegationList) ContainsZone(host string) bool {
	for _, z := range d.Items {
...
// Find compares z with items in list and return existing item otherwise returns nil
func (d *ZoneDelegationList) Find(z *ZoneDelegation) *ZoneDelegation {
	for _, item := range d.Items {
		if item.Spec.LoadBalancedZone == z.Spec.LoadBalancedZone && item.Spec.ParentZone == z.Spec.ParentZone {
			return &item
		}
	}
	return nil
